...
try {
    ...
    if (!isset($_POST['jeniskenderaan'])) {
        throw new Exception('Missing required fields');
    }

    $jeniskenderaan = trim($_POST['jeniskenderaan']);
    ...
    $check->execute([$jeniskenderaan]);
    if ($check->fetchColumn() > 0) {
        throw new Exception('Jenis kenderaan already exists');
    }

    // Insert new record
    $stmt = $pdo->prepare("INSERT INTO ttempah_jenis (jenis_kenderaan) VALUES (:jeniskenderaan)");
    $stmt->execute([':jeniskenderaan' => $jeniskenderaan]);
