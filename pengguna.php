        ...
        if (empty($nokp) || empty($nama) || empty($email) || empty($password)) {
            ...
            if ($userExists) {
                ...
                try {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO penggunajkn (nokp, nama, email, idptj, bahagian, unit, jawatan, gred, nohp, password, role, status)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'staff', ?)");

                    $result = $stmt->execute([$nokp, $nama, $email, $idptj, $bahagian, $unit, $jawatan, $gred, $nohp, $password, $status]);
                    ...
                    if ($result) {
                        ...
                        $nokp = $nama = $email = $idptj = $bahagian = $unit = $jawatan = $gred = $nohp = $password = "";
                    } else {
...
function searchRecords($conn, $searchParams)
{
    $query = "SELECT 
              ...
              tg.gred AS namaGred
            FROM penggunajkn p
            LEFT JOIN tptj tp ON p.idptj = tp.id
            LEFT JOIN tbahagian tb ON p.bahagian = tb.id
            LEFT JOIN tunit tu ON p.unit = tu.id
            LEFT JOIN tgred tg ON p.gred = tg.id
            WHERE 1=1";
...
<body>
                            ...
                            <?php while ($record = $result->fetch_assoc()):
                                ...
                                <tr>
                                    ...
                                    <td class="jawatan" style="font-size: 15px;">
                                        <strong><?= htmlspecialchars($record['jawatan']) ?></strong><br>
            ...
            <div class="container">
                ...
                <form id="editForm" method="POST">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" id="displayEmail" name="email" required
                                placeholder="Contoh: nama@moh.gov.my">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" id="displayJawatan" name="jawatan" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label for="idptj">Ptj</label>
                            <select id="displayBahagian" name="idptj" onchange="loadBahagian2()">
                                <option value="">-- Sila Pilih --</option>
                                <?php
                                $ptjQuery = $pdo->query("SELECT id, namaptj FROM tptj ORDER BY namaptj");
                                while ($ptj = $ptjQuery->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <div class="form-group">
                            <label for="bahagian">Bahagian</label>
                            <select id="displayUnit" name="bahagian" onchange="loadUnit2()">
                                <option value="">-- Sila Pilih --</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label for="peranan">Peranan <span>*</span></label>
                            <input type="text" id="displayPeranan" name="peranan" readonly>
                        </div>
                        <div class="form-group">
                            <label for="status">Status <span>*</span></label>
    ...
    <script>
        ...
        function showViewModal(el) {
            ...
            document.getElementById('displayPeranan').value = perananMap[peranan] || peranan;
